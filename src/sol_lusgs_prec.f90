...
subroutine sol_lusgs_std_prec
    ...
    do nc=1,nblkcoms
        ...
        call lusgs_std_prec_foreward(nb)
        call assign_bc_var_nb(nb,mb_dq,1,neqn,0,dq0)

        call lusgs_std_prec_backward(nb)
        call assign_bc_var_nb(nb,mb_dq,1,neqn,0,dq0)
...
end subroutine sol_lusgs_std_prec

subroutine sol_lusgs_std_prec_sp
    use mod_kndconsts, only : kind_int,kind_real
    use mod_constants, only : zero
    use mod_constants, only : ntimeadv_steady
    use mod_variables, only : nghnode,nstep
    ...
    do nc=1,nblkcoms
        ...
        call lusgs_std_prec_foreward_sp(nb)
        !call assign_bc_var_nb(nb,mb_dq,1,neqn,0,dq0)

        call lusgs_std_prec_backward_sp(nb)
        !call assign_bc_var_nb(nb,mb_dq,1,neqn,0,dq0)
...
end subroutine sol_lusgs_std_prec_sp

subroutine lusgs_std_prec_foreward(nb)
    use mod_kndconsts, only : kind_int,kind_real
    ...
    real(kind_real)            :: visl,vist,vis,length,rca,rcb,rcc,sumSpectralRadius,diagElement
#ifdef OMP_IMP
    ...
    rdt  => mb_rdt(nb)%fld
    dt   => mb_dt(nb)%fld
    !prec_inv => mb_prec_inv(nb)%fld   !<预处理矩阵Γ的逆矩阵
    !nonprec => mb_nonprec(nb)%fld     !<守恒变量对原始变量的转换矩阵P
    !diagInv => mb_prec_diag_inv(nb)%fld   !<对角矩阵的逆矩阵
...
!$OMP do schedule(static)
    ...
    do i=st(1),ed(1),dijk
        ...
        !获取网格点上的预处理矩阵和变量转换矩阵
        !point_prec_inv_i(j,k) : Γinv_i(j,k)
        !point_nonprec_i(j,k): P_i(j,k)
        !do m=1,4
        !    point_nonprec_i(m) = nonprec(m)%r3d(i1,j,k)
        !    point_nonprec_j(m) = nonprec(m)%r3d(i,j1,k)  
        !    point_nonprec_k(m) = nonprec(m)%r3d(i,j,k1)
        !end do
        ...
        if (nvis > nvis_euler) then
            ...
            do m=1,neqn
                rhs0(m) = rhs0(m) + dfvi(m) + dfvj(m) + dfvk(m)
            end do
        end if        
        ...
        do l = 1,5
            ...
            do m = 1,5
                oned_index = 5*(l-1)+m
                dq(l)%r3d(i,j,k) = dq(l)%r3d(i,j,k) + diagInvMatrix(oned_index)*rhs0(m)
            end do
        end do
...
!$OMP end parallel
end subroutine lusgs_std_prec_foreward

subroutine lusgs_std_prec_foreward_sp(nb)
    use mod_kndconsts, only : kind_int,kind_real
    use mod_constants, only : zero,one,half,nprec_non
    use mod_constants, only : nvis_euler,nsw_dir_close
    use mod_datatypes, only : fld_array_t
...
!$OMP end parallel
end subroutine lusgs_std_prec_foreward_sp

subroutine lusgs_std_prec_backward(nb)
    use mod_kndconsts, only : kind_int,kind_real
    use mod_constants, only : zero,one,half
    use mod_constants, only : nvis_euler,nsw_dir_close,nprec_non
    use mod_datatypes, only : fld_array_t
    use mod_variables, only : nvis,nsw_kdir,nprec,fsw_kdir
    use mod_variables, only : refbeta,gamma
    use mod_fieldvars, only : mb_top,mb_sxyz,mb_pv
    ...
    !type(fld_array_t), pointer :: prec_inv(:),nonprec(:),diagInv(:)
    real(kind_real)            :: point_nonprec_i(1:4),point_nonprec_j(1:4),point_nonprec_k(1:4)
    real(kind_real)            :: dUi(1:5),dUj(1:5),dUk(1:5),dfvi(1:5),dfvj(1:5),dfvk(1:5)
    real(kind_real)            :: gamaInvDfi(1:5),gamaInvDfj(1:5),gamaInvDfk(1:5),gamaInvDf(1:5)
    real(kind_real)            :: gamaInvDfvi(1:5),gamaInvDfvj(1:5),gamaInvDfvk(1:5),gamaInvDfv(1:5)
    real(kind_real)            :: gamaInvRhs(1:5)
    real(kind_real)            :: diagInvMatrix(1:25),prim(1:5)
    real(kind_real)            :: rca,rcb,rcc,length,visl,vist,vis,diagElement,sumSpectralRadius
...
!$OMP do schedule(static)
    ...
    do i=st(1),ed(1),dijk
        ...
        !todo odia需要替换成D矩阵的逆矩阵
        !odia = one/rdt(1)%r3d(i,j,k)
        !diagInvMatrix(:) = diagInv(:)%r3d(i,j,k)
...
!$OMP end parallel
end subroutine lusgs_std_prec_backward

subroutine lusgs_std_prec_backward_sp(nb)
    use mod_kndconsts, only : kind_int,kind_real
    use mod_constants, only : zero,one,half
    use mod_constants, only : nvis_euler,nsw_dir_close,nprec_non
    use mod_datatypes, only : fld_array_t
...
!$OMP end parallel
end subroutine lusgs_std_prec_backward_sp
