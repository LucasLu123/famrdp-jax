...
!>
!! @brief 建立全场所有奇异连接点（简称奇点）上的拓扑信息
!! @author 王勇献
!! @remark 2012年04月01日 采用树结构搜索奇点，其时间开销几乎可忽略不计
!>
subroutine search_singulars()
    ...
    ! step 4: 筛选过滤掉不需要的等价集（过滤基数不超过2的等价集）
    do nbs = 1, nblocks
    ...
    ! step 6: 填充 nsimp_pnts_local, 以及 simp_arr 的 gids 域
    ...
    ! step 7: 填充simp_arr 的 fsw 域
    call fill_simp_fsw_field()

    ! step 8: 准备消息映射表, 消息缓冲区
...
!                end if
    ...
    subroutine divide_into_disjoint_sets(box_group, changed)
        ...
        type(box_t) :: outbox(5)    ! 两个线段最多分解成5个不交子集
        ...
        loop_block : do nbs=1,nblocks
            ...
            ! 建立单向循环链表, bnp_head 总指向辅助用的头结点, bnp_tail 总指向链表的尾结点.
            ! 当列表为空时, head = tail = head%next = tail%next
            bnp_head%next => bnp_head
            bnp_tail => bnp_head
            call list_append(bnp_tail, tree_root)

            ! 对链表中所有项，逐个考察其跟其余项是否相交
            bnp_this => bnp_head
    ...
    subroutine collect_all_singular_segment(box_group, changed)
        ...
        ! 至此，已经覆盖了所有的奇点坐标信息（做完了加法），以后只需要做减法
    ...
    subroutine build_segment_set(box_group)
        ...
        ! 构造全局数组 sim_top_tmp (数组中每一项为一单点集合)
    ...
    subroutine fill_simp_fsw_field()
        ...
        do i = 1, nsimp_arr    !! 奇点壁面优先原则(rhs也需要吗？？？)
            npnts = simp_arr(i)%npnts

            succ = .false.  ! 表示没有哪个点位于物面上
            simp_arr(i)%pnts(6, :) = 0
            do j = 1, npnts
                nbs = simp_arr(i)%pnts(1, j)
